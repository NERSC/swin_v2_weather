...
#        self.head = nn.Linear(embed_dim, self.out_chans*self.patch_size[0]*self.patch_size[1], bias=False)
        ...
        p2 = self.patch_size[0] * self.patch_size[1]
        scale = params.decoder_scale
        self.upsample = nn.ConvTranspose2d(embed_dim, embed_dim//scale, kernel_size=self.patch_size[0], stride=self.patch_size[0], padding=0, bias=True)
        self.up_drop = nn.Dropout(p=drop_rate)
        self.head = nn.Conv2d(embed_dim//scale, self.out_chans, kernel_size=1, stride=1, padding=0)
