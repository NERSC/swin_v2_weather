...
class SwinTransformerV2Cr(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        if self.full_pos_embed:
            self.pos_embed_x = nn.Parameter(torch.randn(1, embed_dim, patch_grid_size[0], patch_grid_size[1]) * .02)
    ...
    def patch_merge(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """ This function merges patches created from HRRR and patches created from ERA5. Improve this to implement spatial merging with differing numbers of patches for HRRR and ERA5. """
    ...
    def forward_features(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        z = self.patch_merge(x, y)

        print("z", z.shape)

        if self.checkpoint_stages and not torch.jit.is_scripting():
            z = checkpoint(self.stages, z, use_reentrant=False)
        else:
            z = self.stages(z)
        return z
